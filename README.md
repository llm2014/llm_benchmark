...
4. 因为题目会每月增减，每个模型的成绩在每个月榜单中会有正负3分左右的变化，属于正常现象。大致排序保持稳定。
...
64 三维重建：根据多个方向信息重建三维物体
65 长文总结：从文本中提炼关键数据进行统计汇总
...
74 隐性约束：解题的关键在于跳出题面
75 多轮计算：有限轮数，使用指定操作达到目标值
76 多解诱导：在一个只有唯一解的问题上诱导模型输出多组解
77 知识遍历：需要遍历一个领域的所有知识，找到符合条件的值
78 真解探索：在一个受约束的有限解空间中大胆试探，找到真正唯一解
```
